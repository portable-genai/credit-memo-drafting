...
nineteen acts in :mod:`scripts.demo_console.acts`, the same acts
...
def stage(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Stage]:
    ...
    nineteen ERRORS rather than the honest skip (or, when a browser was promised, the
...
def test_the_demo_covered_every_act(stage: Stage) -> None:
    ...
    assert len(ACTS) >= 19, "acts have gone missing from the walkthrough"
