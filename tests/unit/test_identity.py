...
from credit_memo.adapters.onprem.identity import OnPremIdentityAdapter
...
def test_persona_header_is_case_insensitive() -> None:
    # RequestContext.header() lower-cases the lookup name, and get_principal (api/security.py)
    # lower-cases the header KEYS when building the context, so a host that sends the mixed-case
    # wire header "X-Dev-Persona" still resolves. Emulate that normalization here.
    wire_headers = {"X-Dev-Persona": "other-tenant"}
    ctx = RequestContext(headers={k.lower(): v for k, v in wire_headers.items()})
    principal = _adapter().resolve(ctx)
...
def test_iap_unset_refuses_before_the_assertion_is_read(
    ...
    """
    from credit_memo.adapters.gcp.iap_identity import IapIdentityAdapter

    monkeypatch.delenv("CREDIT_MEMO_IAP_AUDIENCE", raising=False)
    adapter = IapIdentityAdapter(Settings(profile="gcp"))
    with pytest.raises(IdentityError, match="CREDIT_MEMO_IAP_AUDIENCE"):
        adapter.resolve(RequestContext(headers={}))
    with pytest.raises(IdentityError, match="CREDIT_MEMO_IAP_AUDIENCE"):
...
def test_iap_configured_empty_refuses_at_construction(
    ...
    monkeypatch.setenv("CREDIT_MEMO_IAP_AUDIENCE", value)
    with pytest.raises(ConfiguredEmptyError, match="CREDIT_MEMO_IAP_AUDIENCE"):
