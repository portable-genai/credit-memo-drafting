...
``resolve_bind_host``: the Dockerfile CMD is ``uvicorn credit_memo.api.app:app --host 0.0.0.0
--port ${PORT}``, so a bound that lives only in ``main()`` never runs in a shipped process.
Until this file existed, ``CREDIT_MEMO_PROFILE=local`` served the seeded persona list, groups
and tenant included, to any peer that could reach the port, and a LAN peer could then act as
the credit approver by naming it in a header.

The second half is WHAT the guard is derived from. It must not be a service credential:
``CREDIT_MEMO_S2S_TOKEN`` authenticates a calling SERVICE and no end user, so setting one is
...
_ENV = (
    "CREDIT_MEMO_PROFILE",
    "CREDIT_MEMO_S2S_TOKEN",
    "CREDIT_MEMO_ALLOW_INSECURE_DEMO",
...
def _app_under(monkeypatch: pytest.MonkeyPatch, **env: str | None) -> Any:
    ...
    module = importlib.import_module("credit_memo.api.app")
...
def test_the_same_posture_still_serves_a_loopback_peer(
    ...
    app = _app_under(monkeypatch, CREDIT_MEMO_PROFILE="local")
...
def test_a_verifying_binding_stands_the_guard_down(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    app = _app_under(
        ...
        CREDIT_MEMO_IAP_AUDIENCE="/projects/000/global/backendServices/000",
        CREDIT_MEMO_S2S_TOKEN="s3cret",
    )
...
def test_the_opt_out_is_the_documented_one(monkeypatch: pytest.MonkeyPatch) -> None:
    """CREDIT_MEMO_ALLOW_INSECURE_DEMO=1 is the single acknowledged exposure, as at bind time.
    ...
    app = _app_under(
        ...
        CREDIT_MEMO_ALLOW_INSECURE_DEMO="1",
    )
...
def test_the_guard_is_not_derived_from_the_service_credential() -> None:
    ...
    source = Path(__file__).resolve().parents[2] / "src" / "credit_memo" / "api" / "app.py"
