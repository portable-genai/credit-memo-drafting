...
able to import the vertical-neutral kernel **without** dragging in the credit-underwriting
artifacts it is going to rewrite.

So the primary assertion is executed, not read: a fresh interpreter imports
``credit_memo.domain.kernel`` and reports whether ``credit_memo.domain.models`` ended up
...
from credit_memo.domain import kernel, models

SRC = Path(__file__).resolve().parents[2] / "src"
KERNEL_PATH = SRC / "credit_memo" / "domain" / "kernel.py"
...
# The credit-underwriting artifacts a fork rewrites. None may live in the kernel.
VERTICAL_NAMES = (
    "Borrower",
    "Covenant",
    "CovenantStatus",
...
def test_importing_the_kernel_does_not_import_the_vertical_models() -> None:
    ...
    imported = _import_probe("credit_memo.domain.kernel")["modules"]
    assert "credit_memo.domain.kernel" in imported
    assert "credit_memo.domain.models" not in imported, (
...
def test_the_vertical_models_do_import_the_kernel() -> None:
    ...
    imported = _import_probe("credit_memo.domain.models")["modules"]
    assert "credit_memo.domain.kernel" in imported
...
def test_kernel_source_has_no_intra_package_imports() -> None:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            assert not (node.module or "").startswith("credit_memo"), node.module
        elif isinstance(node, ast.Import):
            for alias in node.names:
                assert not alias.name.startswith("credit_memo"), alias.name
