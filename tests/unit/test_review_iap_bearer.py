...
from credit_memo.adapters.platform.review_router import PlatformReviewRouter
...
from credit_memo.domain.models import Borrower, CreditMemo
...
def _memo() -> CreditMemo:
    return CreditMemo(
        borrower=Borrower(id="borr-acme", name="Acme Manufacturing (FICTIONAL)"),
        summary="Leverage covenant breached.",
    )


def _route(router: PlatformReviewRouter) -> None:
    router.route(_memo(), maker="officer@bank.test", tenant="demo-bank")
