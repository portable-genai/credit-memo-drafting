...
``CREDIT_MEMO_PROFILE`` directly can reintroduce the whole class with its own permissive
...
from credit_memo.adapters.local.identity import (
...
def test_only_the_resolver_reads_the_profile_variable_from_the_environment() -> None:
    ...
    assert not offenders, (
        ...
        "so an unset CREDIT_MEMO_PROFILE can again be read as consent:\n" + "\n".join(offenders)
    )


def test_the_settings_file_supplies_no_literal_profile_fallback() -> None:
    """``${CREDIT_MEMO_PROFILE:-local}`` in YAML is the same fail-open, one layer down.
    ...
    assert "${CREDIT_MEMO_PROFILE:-}" in line, line
...
def test_a_configured_empty_profile_refuses_instead_of_inheriting_local(value: str) -> None:
    with pytest.raises(ConfiguredEmptyError, match="CREDIT_MEMO_PROFILE"):
        resolve_profile({"CREDIT_MEMO_PROFILE": value})
...
def test_a_deliberate_profile_is_carried_through_unchanged() -> None:
    choice = resolve_profile({"CREDIT_MEMO_PROFILE": "gcp"})
...
def test_an_unknown_or_miscapitalised_profile_is_refused(value: str) -> None:
    with pytest.raises(ValueError, match="CREDIT_MEMO_PROFILE"):
        resolve_profile({"CREDIT_MEMO_PROFILE": value})
...
def test_seeded_personas_are_refused_under_an_inherited_profile() -> None:
    """The no-auth credit-approver persona needs consent, not a missing env var."""
    inherited = Settings(profile="local", profile_explicit=False)
    with pytest.raises(LocalPersonaProfileError, match="CREDIT_MEMO_PROFILE"):
