...
#   P-07 (immutable audit / WORM, rule R2): the audit log is routed to a Cloud Logging
...
#   P-05 (residency): bucket location is asia-southeast1.
...
resource "google_logging_project_bucket_config" "worm_audit" {
  ...
  location       = var.region         # asia-southeast1 (P-05)
  bucket_id      = "credit-memo-worm" # matches settings.yaml logging.bucket
  description    = "WORM audit bucket for B2 Credit-Memo Assistant (locked, ~7y)."
...
resource "google_logging_project_sink" "audit_to_worm" {
  project     = var.project_id
  name        = "credit-memo-audit-to-worm"
  description = "Routes the credit-memo-audit log to the locked WORM bucket."
  ...
  filter = <<-EOT
    logName="projects/${var.project_id}/logs/credit-memo-audit"
...
# --------------------------------------------------------------------------- #
# Enable Data Access audit logs (DATA_READ) so every read of the borrower
# evidence, the memo and the audit store itself is itself audited (P-07).
# ADMIN_READ and DATA_WRITE are on by default; we add DATA_READ explicitly.
# --------------------------------------------------------------------------- #
