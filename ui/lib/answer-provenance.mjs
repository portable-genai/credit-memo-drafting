...
//
// This console calls its service directly at `API_BASE` (the service's own origin standalone, a
// rooted path on the portal's origin when embedded; see `lib/api-base.mjs`), so the service names
// both headers in `Access-Control-Expose-Headers` on every response: a cross-origin response
// hides every header not exposed, and the pills would stay on the configured model forever with
// every test green.
//
