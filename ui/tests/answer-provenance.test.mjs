...
const STANDALONE = "http://localhost:8093";
const PORTAL = "/credit-memo/api";
...
test("only this console's own API base counts, standalone or under the portal", () => {
  assert.equal(isConsoleApi(STANDALONE + "/v1/credit-memo", HERE, STANDALONE), true);
  assert.equal(isConsoleApi(new URL(STANDALONE + "/healthz"), HERE, STANDALONE + "/"), true);
  assert.equal(isConsoleApi({ url: STANDALONE + "/v1/credit-memo" }, HERE, STANDALONE), true);
  assert.equal(isConsoleApi("http://elsewhere.test:8093/v1/credit-memo", HERE, STANDALONE), false);
  assert.equal(isConsoleApi("/credit-memo/api/v1/credit-memo", HERE, PORTAL), true);
  assert.equal(isConsoleApi("/credit-memo/apisomething", HERE, PORTAL), false);
...
test("the wrapper reports an answer from the console's API and ignores other origins", async () => {
  ...
  await window.fetch(STANDALONE + "/v1/credit-memo", { method: "POST" });
  await window.fetch("http://elsewhere.test/v1/credit-memo");
  stop();
...
test("the wrapper is installed once however many listeners, and restored by the last", async () => {
  ...
  await window.fetch("/credit-memo/api/v1/credit-memo");
  stopFirst();
