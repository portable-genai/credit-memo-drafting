...
Rewrites the package name, the CLI entry point, the ``CREDIT_MEMO_`` env-var prefix, and the
...
changed), then run the gate. See docs/ADOPTING.md for the full checklist (fixtures, the
BigQuery peer set, IdP wiring, region) that this script does NOT do for you.
"""
...
_OLD_PACKAGE = "credit_memo"
_OLD_CLI = "credit-memo"
_OLD_ENV_PREFIX = "CREDIT_MEMO_"
_OLD_RESOURCE = "credit-memo-drafting"
_OLD_DIST = "credit-memo-drafting"
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``CREDIT_MEMO_`` when it prefixes an UPPER token
        # (CREDIT_MEMO_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument("--resource", required=True, help="new resource id stem, e.g. acme-kyc-agent")
    ...
    print("Then finish the human steps in docs/ADOPTING.md (fixtures, peer set, IdP, region).")
    return 0
