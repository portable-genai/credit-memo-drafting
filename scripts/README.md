...
| `credit_memo_console_walkthrough.py` | The **full business walkthrough**: one deal through the REAL console and service in seventeen acts, presenter-paced. Starts both servers itself. Prints what to say and waits for a key before each act and at the beats inside one; `--act NAME\|N` presents a single use case (its predecessors run silently first), `--list` names them all. Every act asserts what it shows, and the same acts run headless as `make demo-console`. See [`docs/demo-use-cases.md`](../docs/demo-use-cases.md). |
