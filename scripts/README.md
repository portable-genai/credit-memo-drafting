...
| `credit_memo_demo_playwright.py` | A **presenter-controlled** Playwright walkthrough of the live server: it prints the business points to make for each step, then waits for you to press Enter before performing it. |
| `credit_memo_console_walkthrough.py` | The **full business walkthrough**: one deal through the REAL console and service in eighteen acts, presenter-paced. Starts both servers itself. Prints the business points to make, in order, and waits for a key before each act and at the beats inside one; `--act NAME\|N` presents a single use case (its predecessors run silently first), `--list` names them all. Every act asserts what it shows, and the same acts run headless as `make demo-console` and in
CI's `demo-browser` job. See [`docs/demo-use-cases.md`](../docs/demo-use-cases.md). |
| `demo_console/` | The acts, narration format, fixtures, locators, server harness and evidence capture the walkthrough and `tests/browser/test_console_use_cases.py` share. Changing an act changes both. |
...
The walkthrough is **paced by you**: it prints the next step's business points as a
numbered list, waits for you to press **Enter**, then clicks **Next** and spotlights the
panel to look at. Each point is a phrase to say; its justification prints underneath in a
dimmer hand, there for the question the phrase provokes rather than to be read out. The six
steps are: memo built (summary) -> financial analysis -> covenants (deterministic status) ->
