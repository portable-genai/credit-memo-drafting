...
Mapping, mirroring :class:`~credit_memo.adapters.gcp.gemini_llm.GeminiLLMAdapter`:
...
Attached documents (``request.documents``) are refused: the local model reads text only,
and under ``live`` spread extraction binds the CSV adapter, so no caller sends one.
...
class LocalModelLLMAdapter:
    ...
    def generate(self, request: LlmRequest) -> LlmResponse:
        ...
        if request.documents:
            # Under live, spread extraction binds the CSV adapter, so no caller attaches a
            # file. One that did would otherwise be answered from the prompt alone, as if
            # the model had read a statement it never saw.
            raise NotImplementedError(
                "the local model reads text only; attached documents are sent to Gemini on "
                "the managed profiles, and the live profile extracts spreads from CSV"
            )
