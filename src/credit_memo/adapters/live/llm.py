...
The ``live`` profile's generator. Memo prose, covenant reasoning and risk
flags are produced by a model running on the operator's own machine; the borrower
evidence in those prompts never leaves it.
...
class GemmaLocalLLMAdapter:
    ...
    def classify(self, text: str, labels: list[str]) -> str:
        ...
        try:
            ...
            # Triage only routes; a server blip must not fail a memo build.
            return labels[0]
