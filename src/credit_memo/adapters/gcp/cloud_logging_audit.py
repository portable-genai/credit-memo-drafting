...
``credit-memo-audit`` log to a **locked log bucket** (WORM, ~7-year retention), so credit
records are write-once and immutable (the regulator-grade audit guarantee).
...
writes. No borrower PII should ever reach this code path.
