...
In the full platform deployment, immutable credit-memo audit records are written through
the shared ``agent-observability`` service (WORM Cloud Logging locked bucket + Cloud
Trace + FinOps) instead of B2 calling Cloud Logging directly. This adapter implements
...
class RemoteAuditError(CreditMemoError):
