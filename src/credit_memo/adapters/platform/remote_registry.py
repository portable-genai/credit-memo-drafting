...
In the full platform deployment, B2's A2A AgentCard is published to and resolved through
the shared ``agent-registry`` service rather than served only from B2's own
...
class RemoteRegistryError(CreditMemoError):
...
class RemoteRegistryAdapter:
    ...
    def register(self, card: AgentCard) -> None:
        ...
        response = self._post(url, to_jsonable(card))
    ...
    def list(self) -> list[AgentCard]:
        ...
        if response.status_code // 100 != 2:
            self._fail(url, response)
        return [self._parse_card(item) for item in (response.json() or ())]
    ...
    def _parse_card(body: dict) -> AgentCard:
        ...
        return AgentCard(
            ...
            skills=skills,
            provider=str(body.get("provider", "credit-memo-drafting")),
        )
