...
Builds the review from the escalated memo and submits it to the human-review-console service
intake (``POST /v1/service/reviews``). The human-review-console base URL comes from the
environment (``HUMAN_REVIEW_URL``). It is bound under the ``gcp`` and ``platform`` profiles
...
class PlatformReviewRouter:
    ...
    def __init__(
        ...
        *,
        ...
        # emptied or backend-path audience is a named refusal, not a failed hand-off per memo.
    ...
    def route(self, memo: CreditMemo, *, maker: str, tenant: str = "") -> None:
        self._client().submit(
            memo_to_review(memo, maker=maker, tenant=tenant), actor="doc2-credit-memo"
        )
