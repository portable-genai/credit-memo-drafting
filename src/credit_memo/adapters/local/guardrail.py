...
The ``local`` profile's stand-in for **Model Armor** (and the A1 guardrail gateway): a
deterministic heuristic that allows benign input and BLOCKS on prompt-injection /
jailbreak patterns (e.g. "ignore all previous instructions", "exfiltrate", "system
prompt"). This deterministically blocks the malicious test input and allows the benign
one, so the blocked-path tests can drive the real adapter by feeding malicious vs benign
text. There is no Google emulator for Model Armor, so this path is unconditional.
"""
