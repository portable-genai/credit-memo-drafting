...
``contextlib.nullcontext`` and ``record_token_usage`` keeps usage in memory, so domain
code that wraps work in ``tracer.span(...)`` runs unchanged with no observability backend
wired up. SDK-free and unconditional. Message content is never captured (P-04); only
token counts are retained, for optional local FinOps inspection.
"""
