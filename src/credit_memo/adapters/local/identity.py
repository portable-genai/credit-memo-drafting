...
# The persona id is the suffix of ``source`` after the colon. Entitlement group names use
# this repo's credit-underwriting domain (credit-analyst / credit-approver / audit), and one
# persona is deliberately in a different tenant so per-tenant authZ is demoable offline.
_PERSONAS: tuple[Principal, ...] = (
    ...
    Principal(
        ...
        principals=("group:credit-analyst", "group:risk", "group:credit-approver"),
        tenant="demo-bank",
        assurance="local-demo",
...
#: (fully offline) and ``live`` (real SEC EDGAR grounding plus a local model server). Every
#: other profile resolves identity from a verified IAP assertion.
...
class LocalPersonaIdentityAdapter:
    ...
    These personas are an UNAUTHENTICATED grant of the credit-analyst and credit-approver
    ...
    the environment) ``CREDIT_MEMO_PROFILE`` or the settings file must actually have named it.
    A missing variable therefore fails closed instead of serving an underwriting assistant
    with dev approvers.
    """

    #: The persona arrives on a header the CALLER writes, so a caller chooses who it is. That
    #: is a picker, not authentication, and the exposure guard reads this declaration to keep
    #: the routes it serves off the LAN.
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "CREDIT_MEMO_PROFILE is not set and config/settings.yaml names no profile, so "
                "the local profile was inherited rather than chosen; seeded dev personas grant "
                "the credit-analyst and credit-approver entitlements with no authentication "
                "and are refused. Set CREDIT_MEMO_PROFILE=local deliberately for a dev or demo "
                "run, or CREDIT_MEMO_PROFILE=gcp for a real deployment."
            )
