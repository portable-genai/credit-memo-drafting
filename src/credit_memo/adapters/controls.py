...
(``CREDIT_MEMO_GUARDRAIL``, ``CREDIT_MEMO_PII_REDACTION``,
``CREDIT_MEMO_REVIEW_ROUTING``), the container binds one of these instead of the profile's
...
* :class:`DisclosingRedaction` notes whether redaction changed the case the analyst gave
  (borrower, sector, documents), which the console discloses rather than drafting a memo
  about a case the analyst did not quite state without saying so.
* :class:`RecordingReviewRouter` records whether a required hand-off reached the console,
  ...
  audited memo must not fail on a console outage, but it is no longer invisible: the
...
from ..domain.models import CreditMemo, Direction, GuardrailVerdict, RedactionResult
...
class RecordingReviewRouter:
    ...
    def route(self, memo: CreditMemo, *, maker: str, tenant: str = "") -> None:
        if not getattr(self._inner, "enabled", True):
            ...
            return
        try:
            self._inner.route(memo, maker=maker, tenant=tenant)
