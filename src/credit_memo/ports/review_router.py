"""ReviewRouterPort: the boundary that routes an escalated credit memo to Hrz7 (rule R8).
...
says a producer that sets ``requires_human_review`` MUST route the item to the Hrz7 Human-Review &
...
class ReviewRouterPort(Protocol):
    def route(self, memo: CreditMemo, *, maker: str, tenant: str = "") -> None:
        """Route an escalated memo to Hrz7 for human review (idempotent per borrower is ideal)."""
        ...
