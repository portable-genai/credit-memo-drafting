...
The Protocol itself is declared ONCE, in :mod:`hex_service_kit.identity`, and re-exported here
rather than restated. A Protocol copied into N repositories is N Protocols, and only one of them
...
#   and an inherited one bind the same seeded personas, ``live`` binds them too while reading
#   as a non-local profile string, and a client's own IdP adapter can be bound under
