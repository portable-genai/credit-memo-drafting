...
@dataclass(frozen=True)
class LiveSettings:
    """The ``live`` profile: real grounding on SEC EDGAR + a local model server.

    Generation runs on any OpenAI-compatible ``/chat/completions`` endpoint on the
    operator's machine. Grounding comes from the SEC's public submissions / XBRL
    company-facts APIs (US-government public-domain data), fetched with the polite
    identified User-Agent the SEC requires and cached on disk so a demo never
    re-downloads the same filing data.
    """
