...
#: ``live`` adds real SEC EDGAR grounding answered by the local open-weight model, ``gcp`` and
#: ``platform`` are the managed stacks, ``onprem`` is the fail-fast portability placeholder.
...
#: The name the ``local`` deterministic generator answers under: what ``generator_model``
#: reports for it and what the adapter notes as the model that answered, spelled once so the
#: configured pill and the answered pill can never disagree about the stub.
...
#: sit on the operator's machine; only the optional web-research leg calls Gemini, and only
#: while CREDIT_MEMO_RESEARCH_ENABLED is on. ``onprem`` is not one either.
...
GUARDRAIL_ENV = "CREDIT_MEMO_GUARDRAIL"
PII_REDACTION_ENV = "CREDIT_MEMO_PII_REDACTION"
REVIEW_ROUTING_ENV = "CREDIT_MEMO_REVIEW_ROUTING"
...
@dataclass(frozen=True)
class LiveSettings:
    """The ``live`` profile: real grounding on SEC EDGAR, answered by the local model.

    Grounding comes from the SEC's public submissions / XBRL company-facts APIs
    (US-government public-domain data), fetched with the polite identified User-Agent the
    SEC requires and cached on disk so a demo never re-downloads the same filing data. The
    core model is the fleet's local open-weight model through the shared
    ``hex_service_kit.localmodel`` client, which owns its own settings (``LOCAL_MODEL_URL``,
    ``LOCAL_MODEL``); ``max_output_tokens`` is the ceiling on one of its answers. No cloud
    credentials are needed unless the optional web research is switched on (owner decision,
    2026-09-23).
    """
...
class Settings:
    ...
    def generator_model(self) -> str:
        ...
        models.reasoning``), never a model a flag could swap in. The "hard reasoning" switch
        that once moved this name while the adapter kept calling ``models.reasoning`` is
...
def _refuse_unconfigured_controls(settings: Settings) -> None:
    ...
    Review routing on with no console named would fail every hand-off, one memo at a time,
    behind the domain's best-effort ``suppress``; the Model Armor guardrail on with no
...
class Container:
    ...
    def review_router(self) -> Any:
        """The maker-checker console hand-off (rule R8), switched by ``CREDIT_MEMO_REVIEW_ROUTING``.

        On unless a deployment says otherwise, like every cheap runtime control. Off binds a
        router that submits nothing, and every memo response then says the memo was not
        queued for review, so a deployment that switches it off (this service's evidence is
        deleted after fifteen days, and a console keeps cases indefinitely) does so out loud.

        What does NOT change with the switch: every memo still carries requires_human_review,
        and the standing "decision support, not a credit decision" sentence is unconditional.
