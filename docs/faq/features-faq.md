...
| Concern | Owned by (repo) | `credit-memo-drafting`'s role |
|---|---|---|
...
| Observability + redacted prompt/response audit | `agent-observability` | writes audit events to it; traces spans through it |
| Maker-checker review console for escalations | `human-review-console` (rule R8) | routes a `requires_human_review` memo to it via `review-kit` |
