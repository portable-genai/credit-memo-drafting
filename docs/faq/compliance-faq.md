...
`COMPLIANCE.md` maps the internal P-01..P-12 and R1..R6 (plus R8, the `human-review-console`
routing) controls to concrete code. The build is region-pinned to `asia-southeast1` (Singapore, MAS) with
...
regulator-reference column, and re-review with local counsel. At scale the sibling
...
on the pin in automation. The residency-violation CI gate is the sibling
