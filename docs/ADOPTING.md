...
(`agent-guardrail-gateway`), the governed knowledge base (`enterprise-knowledge-base`), the
agent registry (`agent-registry`), the AI-quality / eval gate (`model-quality-gate`),
observability + audit (`agent-observability`), the maker-checker review console
(`human-review-console`, rule R8), the compliance assistant (`compliance-advisory`), and the
on-prem DLP gate (`onprem-dlp`). The `platform` profile's adapters
