...
| P-05 Data residency | Met | Region defaults to `asia-southeast1` in `config/settings.yaml` and every `infra/terraform` resource derives its location from it; the deploy-time value is validated against the `allowed_regions` residency allowlist, and extending that list is the review point; regional Document AI / Model Armor / BigQuery endpoints. |
